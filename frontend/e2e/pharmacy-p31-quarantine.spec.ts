...
function seedP31() { execFileSync(process.env.E2E_PYTHON ?? process.env.PYTHON ?? 'python', [path.join(repoRoot, 'backend', 'tests', 'e2e_seed_task7.py'), 'seed_p31_scenario'], { cwd: path.join(repoRoot, 'backend'), stdio: 'inherit', env }) }
function snapshotP31() { return JSON.parse(execFileSync(process.env.E2E_PYTHON ?? process.env.PYTHON ?? 'python', [path.join(repoRoot, 'backend', 'tests', 'e2e_seed_task7.py'), 'snapshot_p31'], { cwd: path.join(repoRoot, 'backend'), encoding: 'utf8', env })) }
