...
FACILITY_B_ID = uuid.uuid5(uuid.NAMESPACE_DNS, "hms-e2e-task7-facility-b")
...
async def seed():
    ...
    async with maker() as session:
        ...
        visit = Visit(id=VISIT_ID, patient_id=PATIENT_ID, uhid=patient.uhid, doctor_id=DOCTOR_ID, department_id=DEPARTMENT_ID, facility_id=FACILITY_A_ID, status="IN_CONSULTATION", arrived_at=now, registered_at=now, pre_vital_completed_at=now, doctor_queue_at=now, doctor_called_at=now, consultation_started_at=now)
        ...
        p28_visit = Visit(id=P28_VISIT_ID, patient_id=P28_PATIENT_ID, uhid=p28_patient.uhid, doctor_id=DOCTOR_ID, department_id=DEPARTMENT_ID, facility_id=FACILITY_A_ID, status="CONSULTATION_COMPLETED", arrived_at=now, registered_at=now, pre_vital_completed_at=now, doctor_queue_at=now, doctor_called_at=now, consultation_started_at=now)
        ...
        base_pharmacy_location = PharmacyLocation(id=PHARMACY_LOCATION_A_ID, tenant_id=TENANT_A_ID, facility_id=FACILITY_A_ID, location_code="E2E-BASE", location_name="E2E Base Pharmacy", location_type="PHARMACY", active=True)
        ...
        session.add_all([department, doctor, patient, visit, prescription_patient, prescription_visit, p28_patient, p28_visit, vitals, icd_active, icd_inactive, generic_a, form_a, route_a, manufacturer_a, product_a, product_a_capsule, med_table, med_capsule, med_inactive, supplier_a, base_pharmacy_location])
        await session.flush()
        ...
        visit_b = Visit(id=HOSPITAL_B_VISIT_ID, patient_id=HOSPITAL_B_PATIENT_ID, uhid=patient_b.uhid, doctor_id=HOSPITAL_B_DOCTOR_ID, department_id=HOSPITAL_B_DEPARTMENT_ID, facility_id=FACILITY_B_ID, status="WAITING_FOR_DOCTOR", arrived_at=now, registered_at=now, pre_vital_completed_at=now, doctor_queue_at=now)
...
async def reset_p28_scenario():
    ...
    async with engine.begin() as conn:
        ...
        await conn.execute(text("UPDATE invoices SET pharmacy_dispense_id = NULL WHERE pharmacy_dispense_id IN (SELECT id FROM pharmacy_dispenses WHERE prescription_id = :rx)"), {"rx": P28_PRESCRIPTION_ID})
        await conn.execute(text("UPDATE pharmacy_dispenses SET invoice_id = NULL WHERE prescription_id = :rx"), {"rx": P28_PRESCRIPTION_ID})
...
async def seed_p30_scenario():
    ...
    async with maker() as session:
        ...
        single_visit = Visit(id=P30_SINGLE_VISIT_ID, patient_id=P30_PATIENT_SINGLE_ID, uhid=single_patient.uhid, doctor_id=DOCTOR_ID, department_id=DEPARTMENT_ID, facility_id=FACILITY_A_ID, status="CONSULTATION_COMPLETED", arrived_at=now, registered_at=now)
        multi_visit = Visit(id=P30_MULTI_VISIT_ID, patient_id=P30_PATIENT_MULTI_ID, uhid=multi_patient.uhid, doctor_id=DOCTOR_ID, department_id=DEPARTMENT_ID, facility_id=FACILITY_A_ID, status="CONSULTATION_COMPLETED", arrived_at=now, registered_at=now)
